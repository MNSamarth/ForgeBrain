...
- **Content is heuristic, not AI-generated.** Research, Lesson, Content Director, and Script all use deterministic templates/rules over the curriculum's own curated data — genuinely structured and internally consistent, but not the quality a real LLM-backed implementation would produce. `VertexAiClient` (`vertex/`) is a real interface with zero implementation and zero GCP dependency in `pom.xml` — deliberately, per the task's instruction to wire the abstraction without calling it yet.
...
2. **Wire a real `VertexAiClient`** against Vertex AI (add the SDK dependency to `pom.xml`, implement the interface, replace `ResearchServiceImpl`/`LessonServiceImpl`/`ScriptServiceImpl`'s heuristic bodies with real prompt calls behind the same method signatures — the interfaces don't need to change, only what's behind them). Do this one stage at a time, keeping the heuristic implementation as a fallback/comparison point.
