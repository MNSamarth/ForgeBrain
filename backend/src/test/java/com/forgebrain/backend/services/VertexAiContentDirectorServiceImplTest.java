 ...
 * no real network call is made. Covers: successful Vertex AI generation is parsed and assembled
 * correctly, and every way the client can be "unavailable" (missing config, thrown exception,
 * malformed JSON, incomplete JSON) correctly falls back to the heuristic strategy.
 */
class VertexAiContentDirectorServiceImplTest {
    ...
    private VertexAiContentDirectorServiceImpl service(VertexAiConfig config) {
        return new VertexAiContentDirectorServiceImpl(vertexAiClient, config, objectMapper);
    }
